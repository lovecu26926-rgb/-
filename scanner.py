...
CATEGORIES = ["돌파_52W", "돌파_50", "돌파_20", "눌림목"]
...
def load_fundamentals():
    """
    reversal.csv, supertrend.csv 양쪽 모두 펀더멘털 데이터 병합용으로 로드.
    (더 이상 소스별로 시그널 카테고리를 제한하지 않음 - 골든크로스/추세전환
    삭제로 인해 소스 구분의 의미가 없어졌기 때문)
    """
    fund_map = {}

    sources = [REVERSAL_CSV, SUPERTREND_CSV]
        ...
        try:
            ...
            if signals:
                for s in signals:
                    if not rs_pass(s, rs):
                        continue
                    buckets[s].append((t, rs, vol_ratio, current_price, pos_52w))
