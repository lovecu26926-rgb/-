...
# 설정
# =========================
TREND_CSV = "reversal.csv"          # 추세전환 전용 파일
SUPERTREND_CSV = "supertrend.csv"   # 돌파/눌림/골든 전용 파일
...
# =========================
# 52주 고가 이격도 + 섹터 정보 (수정됨)
# =========================
def calc_52w_position_and_sector(ticker, current_price):
    try:
        t = yf.Ticker(ticker)
        info = t.info
        ...
        if high52 is None or high52 <= 0:
            return None, sector
        pos = float((current_price - high52) / high52 * 100)
        return pos, sector
    except:
        return None, 'N/A'
...
def attach_composite_scores(items, category):
    ...
    for i, it in enumerate(items):
        ticker, primary, vol_ratio, price, pos_52w, sector = it  # sector 언패킹
        p_pct = p_rank.get(i, 50.0)
        v_pct = v_rank.get(i, 50.0)
        if not favors_high:
            v_pct = 100.0 - v_pct
        composite = rs_w * p_pct + vol_w * v_pct
        scored.append((ticker, primary, vol_ratio, price, pos_52w, sector, composite))  # sector 저장
...
# =========================
# CSV 재무 데이터 로드 (경로 매핑 수정됨)
# =========================
def load_fundamentals():
    dfs = []
    # reversal.csv -> "trend" 라벨 (추세전환 전용)
    # supertrend.csv -> "supertrend" 라벨 (돌파/눌림/골든 전용)
    for path, source in [("reversal.csv", "trend"), ("supertrend.csv", "supertrend")]:
...
def scan():
    ...
    for t in tickers:
        ...
        try:
            ...
            # 52W 이격도 + 섹터 함께 받아오기
            pos_52w, sector = calc_52w_position_and_sector(t, current_price)
            ...
            for s in signals:
                ...
                # sector도 함께 저장
                buckets[s].append((t, primary, vol_ratio, current_price, pos_52w, sector))
    ...
    # =========================
    # 메시지 출력 (1번 스타일 + 이모지 제거 + ROE/섹터 앞으로)
    # =========================
    cat_labels = {
        "돌파_52W": "🏆 [돌파] 52주 신고가 🔥",
        "돌파_50":  "🏆 [돌파] 50일 고가",
    ...
    for cat in CATEGORIES:
        ...
        for i, (t, primary, vol_ratio, current_price, pos_52w, sector, composite) in enumerate(items, 1):
            f = fund_map.get(t, {})
            ...
            # 최종 출력 (1번 스타일, 이모지 제거, ROE & 섹터를 RS/VOL 라인으로 이동)
            msg += f"─────────────────────────────\n"
            msg += f"{i}. {t}  ${current_price:.1f}  |  52W {pos_str}\n"
            msg += f"   RS: {rs_str}  |  VOL: {vol_str}  |  ROE: {roe_str}  |  {sector_str}\n"
            msg += f"   EPS YoY: {eps_str} {growth_tag}  |  EPS FWD: {fwd_str}\n"
            msg += f"   EPS QoQ: {qoq_str} {qoq_tag}  |  REV YoY: {rev_str} {rev_tag}\n"
