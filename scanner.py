...
def classify_stock(stock):
    ...
    """
    yoy = to_float(stock.get("yoy", 0))
    rev = to_float(stock.get("rev", 0))
    qoq = to_float(stock.get("qoq", 0))
    fwd = to_float(stock.get("fwd", 0))
...
# ==================== 2. CSV 읽기/쓰기 (to_float 적용) ====================
def read_csv(file_path):
    """CSV를 읽고, 실제 컬럼명을 내부 키로 매핑합니다 (안전 변환)."""
    data = []
    try:
        with open(file_path, 'r', encoding='utf-8-sig') as f:
            ...
            for row in reader:
                mapped = {
                    ...
                    "yoy": to_float(row.get("EPS_Growth_TTM_YoY", 0)),
                    "rev": to_float(row.get("Revenue_Growth_TTM_YoY", 0)),
                    "roe": to_float(row.get("ROE", 0)),
                    "fwd": to_float(row.get("Estimated_EPS_FY", 0)),
                }
                # QoQ 계산 (안전 변환)
                current_q = to_float(row.get("EPS_Current_Quarter", 0))
                next_q = to_float(row.get("EPS_Next_Quarter", 0))
                if current_q != 0:
