...
# =========================
# 🔥 티커 추출 (소문자/공백 나오기 전까지만)
# =========================
def extract_ticker(raw_symbol: str) -> str:
    raw_symbol = raw_symbol.strip().strip('"')

    # 소문자 or 공백/특수문자 나오기 직전까지 대문자+숫자만 추출
    m = re.match(r'^([A-Z0-9]+?)(?=[a-z\s&,.])', raw_symbol)
    if m:
        return m.group(1)

    # fallback: 순수 대문자+숫자 연속
    m = re.match(r'^([A-Z0-9]+)', raw_symbol)
    if m:
        return m.group(1)
    ...
    for path, source_label in sources:
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                ticker = extract_ticker(raw_symbol)  # 🔥 소문자 나오면 즉시 컷
    ...
    for t in tickers:
        try:
            ...
            if df is None or df.empty:
                print(f"${t}: possibly delisted; no price data found")
                continue
