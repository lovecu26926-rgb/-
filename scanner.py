...
def clean_numeric(val):
    if isinstance(val, str):
        ...
        # 2. 🔥 USD 제거 (공백 유무/대소문자 상관없이 모두 제거)
        val = val.replace('USD', '').replace('usd', '').replace('Usd', '')
        
        # 3. 🔥 #ERROR!, N/A 등 에러 문자열 처리
    ...
    for path, source_label in sources:
        ...
        try:
            ...
            for _, row in df.iterrows():
                # 🔥 수정 2: Symbol에서 티커만 추출 (예: "MU Micron" → "MU")
                raw_symbol = str(row["Symbol"]).strip()
                ticker = raw_symbol.split()[0]  # 첫 번째 공백 앞부분만
