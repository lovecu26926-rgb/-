...
def get_integrated_universe():
    ...
    try:
        ...
        for sym in table['Symbol'].dropna():
            symbol = str(sym).strip().replace('.', '-', regex=False)
    ...
    # 3. Russell 1000 (위키피디아는 Russell 1000 Index 명칭 사용)
    try:
...
def scan():
    ...
    if not universe:
        logging.error("스캔할 종목 리스트가 빔. 종료합니다.")
        return

    buy_alerts = []
    chunk_size = 45 # 1000개급 조회를 위한 가장 안정적인 버퍼 크기
    ...
    if not buy_alerts:
        send_telegram(f"📊 <b>[{date_str}]</b> 미국 주요 지수 중 Supertrend 매수 전환 종목 없음")
        return

    lines = [
        f"<b>🎯 Supertrend Buy Signal [{date_str}]</b>",
        f"<i>필터: 주요 3대지수 우량주 (대금 > ${MIN_DOLLAR_VOLUME/1_000_000:.0f}M)</i>",
        ""
    ]
