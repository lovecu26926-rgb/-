#!/usr/bin/env python3
"""
Supertrend Alert Bot - 대용량 유니버스(S&P500, NASDAQ100, Russell핵심) 초고속 배치 버전
"""
...
def get_live_universe():
    """웹에서 S&P 500 및 NASDAQ 100 최신 종목을 동적으로 크롤링합니다."""
    ...
    # 러셀 2000 핵심/대형주 추가 (전체 2000개는 무료 API 통배치 한계를 넘으므로 주요 유동성 종목 위주 조합 권장)
    # 임시 방편으로 기존 리스트 유지 및 수집된 대형주 결합
...
def scan():
    ...
    # ⚠️ 핵심: 2500개 종목을 한꺼번에 묶어서 대량 다운로드 (차단 방지 및 수십배 속도 향상)
    try:
    ...
    for ticker in universe:
        try:
            ...
            # 신호 포착: 전일 하락(-1)에서 오늘 상승(1) 전환 전환 시점만 잡아내기
    ...
    if buy_alerts:
        # 메시지가 너무 길면 텔레그램 자르기 제한이 오므로 끊어서 전송
        chunks = []
