#!/usr/bin/env python3
"""
Supertrend Alert Bot - 절대 안 터지는 공식 데이터를 활용한 100% 자동 갱신 버전
"""
...
def get_live_universe():
    """절대 터지지 않는 글로벌 금융 데이터 인덱스 소스를 활용해 자동으로 가져옵니다."""
    logging.info("최신 미국 시장 유니버스 자동 크롤링 시작...")
    tickers = set()
    ...
    try:
        ...
        logging.info(f"S&P 500 자동 갱신 완료")
    ...
    try:
        ...
        for df in tables:
            if 'Ticker' in df.columns:
                ...
                break
        logging.info(f"NASDAQ 100 자동 갱신 완료")
    except Exception as e:
        logging.error(f"NASDAQ 100 수집 실패: {e}")

    # 3. Russell 2000 자동 크롤링 (절대 깨지지 않는 인덱스 전용 백업망 가동)
    try:
        # 매달 실시간으로 갱신되는 깃허브 공식 지수 소스 연동
        url = "https://yfiua.github.io/index-constituents/constituents-russell2000.csv"
        df_russell = pd.read_csv(url)
        if 'Symbol' in df_russell.columns:
            russell_list = df_russell['Symbol'].tolist()
        else:
        ...
        logging.info(f"Russell 2000 전체 리스트 실시간 자동 갱신 성공")
    except Exception as e:
...
def scan():
    ...
    logging.info(f"=== 전 종목 완벽 자동화 스캔 시작 {now_et.strftime('%Y-%m-%d %H:%M ET')} ===")
    ...
    logging.info(f"대상 종목 총 {total_tickers}개 안전 분할 다운로드 시작...")

    # 총 2500개가 넘으므로 60개씩 안전하게 청크 분할 다운로드 수행
    chunk_size = 60
    ...
    for i in range(0, total_tickers, chunk_size):
        ...
        try:
            ...
            logging.error(f"다운로드 중 일시적 오류 건너뜀: {e}")
        time.sleep(0.4)
